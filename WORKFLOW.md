...
- If the handoff helper reports findings before the local-review cap, inspect the latest artifact under `.codex/review_artifacts/`, fix the valid findings in the same implementation run, rerun the smallest relevant validation, and rerun the handoff helper.
- Findings before the local-review cap are continuation work for the same implementation worker. Keep the issue in `In Progress`; do not stop or expect Symphony to redispatch a fresh worker for those pre-PR fixes.
- Cap the local-review remediation loop at 3 handoff review rounds total. If the third round still reports findings, the handoff helper opens or updates the PR, moves the issue to `In Review`, and escalates the branch to Devin with the latest local-review artifact attached. Do not keep cycling locally after the cap is reached.
