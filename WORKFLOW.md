...
- On a resumed `Ready to Merge` run, verify the linked PR is clean on the current head and branch protection is satisfied, merge with GitHub CLI, confirm the default branch contains the change, move the Linear issue to `Done`, and then run `python3 "$GPU_CFD_CONTROL_REPO_ROOT/scripts/symphony/release_dependents.py" --issue "{{ issue.identifier }}" --workspace "$PWD"` to promote any newly unblocked direct dependents from `Backlog` to `Todo`.
