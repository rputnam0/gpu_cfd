...
def render_issue_description(
    ...
    return (
        ...
        "* If pre-PR local review findings remain before the cap, `scripts/symphony/pr_handoff.py` keeps the issue in `In Progress` so the same implementation worker can remediate them in-place from the latest review artifact.\n"
        "* If pre-PR local review reaches the 3-round cap with findings still open, `scripts/symphony/pr_handoff.py` opens or updates the PR and moves the issue to `In Review` for Devin instead of looping locally forever.\n"
