...
def render_issue_description(
    ...
    return (
        f"Execution task for backlog item `{pr_id}`. Use this issue as the operational tracker and "
        "the repo docs as the technical source of truth.\n\n"
        "## Worker startup stub\n\n"
        "* Start with `AGENTS.md`.\n"
        f"* Open the owning task file below and jump to the exact `## {pr_id}` card.\n"
        "* Read only the cited docs from that card unless repo evidence forces expansion.\n"
        ...
        "* Use native Codex sub-agents only for bounded read-only support work.\n"
        "* Trigger `docs_scout` for docs-plus-code tasks or multi-doc cards, `codepath_scout` for multi-module changes, and `review_payload_scout` on `Rework`, `Refresh Required`, or large review payloads.\n"
        "* Use `gpt-5.4-mini` for helpers, explicitly set that model if you spawn a helper directly, summarize helper findings in the workpad, and keep code edits, tests, Linear updates, PR handoff, and final technical judgment on the main `gpt-5.4` worker.\n\n"
        "## Task routing\n\n"
        f"* Task file: {github_doc_link(repo, metadata.task_file)}\n"
        ...
        f"* Done when: {metadata.done_when}\n"
        ...
        "## Review flow\n\n"
        "* This repo launches only labeled canary issues. The current launch label is `symphony-canary`.\n"
        "* The dispatch wrapper and handoff helper sync canonical control-plane files into the active workspace before dispatch, resume, and review automation.\n"
        "* Run handoff with `python3 \"$GPU_CFD_CONTROL_REPO_ROOT/scripts/symphony/pr_handoff.py\" --workspace \"$PWD\"`.\n"
        "* Local review is finite: remediation pass 1, remediation pass 2, then one final local review pass.\n"
        "* On the third pass, residual local findings become child `Backlog` issues. At the section boundary, GitHub post-merge automation promotes them into a mandatory cleanup sweep before downstream work is released.\n"
        "* GitHub automation moves actionable Devin feedback to `Rework` and clean `BEHIND` or `DIRTY` PRs to `Refresh Required`.\n"
        "* `Rework` and `Refresh Required` both bypass another local Codex review. Return directly to GitHub auto-merge only after the branch contains the latest `origin/main`.\n"
    )
