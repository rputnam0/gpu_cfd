...
def render_issue_description(
    ...
    return (
        ...
        "* Use native Codex sub-agents as bounded recursive research helpers for doc lookup, code-path tracing, nearby test/API summaries, or review-payload inspection. The implementation worker profile explicitly enables multi-agent support and the project child-agent definitions. Prefer `gpt-5.4-mini` for those helpers, and keep code edits, tests, Linear updates, PR handoff, and final technical judgment on the main `gpt-5.4` worker.\n\n"
        "## Source docs\n\n"
