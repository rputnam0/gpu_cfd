...
def render_issue_description(
    ...
    return (
        ...
        "* Open the owning task file below and jump to the exact `## "
        f"{pr_id}` card.\n"
        ...
        "* Use native Codex sub-agents only for narrow read-only support work such as doc lookup, code-path tracing, nearby test/API summaries, or review-payload inspection.\n"
        "* Use `gpt-5.4-mini` for those helpers, explicitly set that model if you spawn a helper directly, and keep code edits, tests, Linear updates, PR handoff, and final technical judgment on the main `gpt-5.4` worker.\n"
        "* Do not fetch full Linear details for already-done dependency issues unless the exact dependency contract is still unclear after reading the repo task card.\n\n"
        "## Task routing\n\n"
