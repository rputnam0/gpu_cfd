...
class ProgressiveDisclosureContractTests(unittest.TestCase):
    ...
    def test_worker_contract_starts_from_agents_and_workpad(self) -> None:
        ...
        self.assertIn("Trigger `docs_scout`", workflow_text)
        self.assertIn("Trigger `codepath_scout`", workflow_text)
        self.assertIn("Trigger `review_payload_scout`", workflow_text)
        ...
        self.assertIn("Refresh Required", workflow_text)
        self.assertIn(
            "Do not return a PR to `In Review` until the branch contains the latest `origin/main`",
            workflow_text,
        )
        self.assertIn("clean `BEHIND` or `DIRTY` PRs", workflow_text)
        self.assertIn("open or update a PR from a conflicted branch", workflow_text)
        ...
        self.assertIn("Trigger `docs_scout`", skill_text)
        self.assertIn("Trigger `codepath_scout`", skill_text)
        self.assertIn("Trigger `review_payload_scout`", skill_text)
        self.assertIn("explicitly set that", skill_text)
        ...
        self.assertIn("Refresh Required", skill_text)
        self.assertIn(
            "return a PR to `In Review` until the branch contains the latest `origin/main`",
            skill_text,
        )
        self.assertIn("open or update a conflicted PR", skill_text)
    ...
    def test_project_codex_config_enables_bounded_multi_agent_support(self) -> None:
        ...
        self.assertIn('config_file = "agents/docs-scout.md"', config_text)
        self.assertIn('config_file = "agents/codepath-scout.md"', config_text)
        self.assertIn('config_file = "agents/review-payload-scout.md"', config_text)
