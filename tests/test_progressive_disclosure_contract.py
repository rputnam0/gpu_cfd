...
class ProgressiveDisclosureContractTests(unittest.TestCase):
    ...
    def test_worker_contract_starts_from_agents_and_workpad(self) -> None:
        ...
        self.assertIn("branch_refresh_required", workflow_text)
        self.assertIn("Do not return a PR to `In Review` until the branch contains the latest `origin/main`", workflow_text)
        self.assertIn("becomes `BEHIND` or `DIRTY`", workflow_text)
        ...
        self.assertIn("Do not fetch full Linear details for already-done dependency issues", skill_text)
        self.assertIn("same implementation worker", skill_text)
        self.assertIn("`Backlog` issue per residual finding", skill_text)
        self.assertIn("branch_refresh_required", skill_text)
        self.assertIn("return a PR to `In Review` until the branch contains the latest `origin/main`", skill_text)
        self.assertIn("became `BEHIND` or `DIRTY`", skill_text)
