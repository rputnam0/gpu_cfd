...
class ReleaseUnblockedDependentsTests(unittest.TestCase):
    @mock.patch("scripts.symphony.github_linear_bridge.update_linear_issue_state")
    @mock.patch("scripts.symphony.github_linear_bridge.linear_graphql")
    def test_promotes_backlog_dependents_when_all_blockers_are_done(
        self,
        linear_graphql: mock.Mock,
        update_linear_issue_state: mock.Mock,
    ) -> None:
        linear_graphql.return_value = {
            "issues": {
                "nodes": [
        ...
        update_linear_issue_state.return_value = {
            "changed": True,
            "previous_state": "Backlog",
            "current_state": "Todo",
            "issue": {"identifier": "PRO-6"},
        }

        released = github_linear_bridge.release_unblocked_dependents("PRO-5")
        ...
        update_linear_issue_state.assert_called_once_with("PRO-6", "Todo")

    @mock.patch("scripts.symphony.github_linear_bridge.update_linear_issue_state")
    @mock.patch("scripts.symphony.github_linear_bridge.linear_graphql")
    def test_does_not_promote_when_any_blocker_is_not_done(
        self,
        linear_graphql: mock.Mock,
        update_linear_issue_state: mock.Mock,
    ) -> None:
        ...
        update_linear_issue_state.assert_not_called()
