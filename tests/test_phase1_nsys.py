...
class Phase1ProfilingTests(unittest.TestCase):
    ...
    def test_run_phase1_nsys_profile_marks_zero_uvm_activity_as_clean(self) -> None:
        def command_runner(command: tuple[str, ...], *, cwd: pathlib.Path, log_path: pathlib.Path) -> int:
            if command[0] == "blockMesh":
                log_path.write_text("Created mesh.\n", encoding="utf-8")
                (cwd / "constant" / "polyMesh").mkdir(parents=True, exist_ok=True)
            ...
            if command[:2] == ("nsys", "stats"):
                ...
                report_payloads = {
                    ...
                    "um_total_sum": _csv_text(("Metric", "Value"), ("Total migrations", "0")),
                    "um_cpu_page_faults_sum": _csv_text(("Metric", "Value"), ("CPU page faults", "0")),
                }
                output_path.write_text(
                    report_payloads.get(report_name, _csv_text(("Metric", "Value"), ("placeholder", "1"))),
                    encoding="utf-8",
        ...
        self.assertEqual(result.status, "pass")
        self.assertEqual(result_payload["uvm"]["cpu_um_faults"], 0)
        self.assertEqual(result_payload["uvm"]["gpu_um_faults"], 0)
        self.assertEqual(result_payload["uvm"]["classification"], "clean")
